# AI 기반 제품 분석 및 타깃 맞춤형 브랜드 아이덴티티 생성기
...
## 3. 테스트 대상 제품
  ...
  항목                                내용
  ----------------------------------- -----------------------------------
  제품명                              Waterpik Ultra Professional WP-670K

  제품 종류                           구강세정기(Water Flosser)

  업종                                구강관리·생활가전

  제품 자료                           제조사 홈페이지 제공 Waterpik Ultra
                                      Professional WP-670K 제품 소개
  -----------------------------------------------------------------------
...
-   **선택 필드:** `tone`, `competitors`, `notes`

이는 과제에서 지정한 브랜드 브리프 입력 구조를 따른다.
...
브랜드 브리프 JSON에 반영하였다. 프로그램은 입력된 브랜드 브리프의 제품
특징, 타깃, 키워드, 톤앤매너 등의 정보를 검증한 후 OpenAI API를 활용하여
브랜드 네이밍, 슬로건, 브랜드 스토리와 컬러를 생성한다.

생성된 컬러 정보는 `matplotlib`을 이용해 컬러 팔레트 PNG로 시각화하며,
브랜드명·컬러·제품 및 타깃 정보를 이미지 생성 단계에 전달하여 로고
시안을 생성한다. 마지막으로 텍스트 결과는 `brand_result.json`, 컬러
팔레트와 로고는 개별 PNG 파일로 출력 폴더에 저장한다.
...
`output/wp670k` 폴더에는 20\~30대 직장인을 대상으로 생성한 최종 브랜드
결과, 컬러 팔레트 및 로고 이미지 3개를 저장하였다.

`output/wp670k_senior` 폴더에는 타깃 변경 비교 테스트를 위해 50\~60대
중장년층을 대상으로 생성한 `brand_result.json`과 `brand_palette.png`를
별도로 저장하였다.
...
### 각 파일의 역할
  ...
  파일                                       역할
  ------------------------------------------ -----------------------------------
  `main.py`                                  사용자 입력 처리, 브랜드 브리프
                                             JSON 유효성 검증, OpenAI API 기반
                                             브랜드 요소 및 로고 생성, 컬러
                                             팔레트 시각화, 결과 저장

  `briefs/wp670k_brief.json`                 WP-670K 제품명, 산업 분야, 20\~30대
                                             직장인 타깃, 핵심 키워드, 톤,
                                             경쟁사 및 제품 특징 저장
  ...
  `requirements.txt`                         `openai`, `python-dotenv`,
                                             `matplotlib` 등 필요한 Python
                                             라이브러리 관리

  `.gitignore`                               `.env`, `.venv`, 캐시 파일 등
                                             제출/Git 제외 항목 지정

  `.env`                                     OpenAI API Key를 환경변수로 관리.
                                             **제출 ZIP에는 포함하지 않음**

  `output/wp670k/brand_result.json`          브랜드명·의미·슬로건·스토리·컬러
                                             결과 저장

  `output/wp670k/brand_palette.png`          브랜드 컬러 팔레트 이미지

  `output/wp670k/logo_01.png` \~             AI가 생성한 3가지 로고 시안
  `logo_03.png`                              

  `output/wp670k_senior/brand_result.json`   중장년층 타깃 브랜드 결과

  `output/wp670k_senior/brand_palette.png`   중장년층 타깃 컬러 팔레트
  ------------------------------------------------------------------------------
...
## 18. 사용 기술 및 개발 환경
  ...
  LLM API/모델                        OpenAI Responses API / GPT-5.6 Luna
...
3.  브랜드 브리프 JSON 파일 경로 `briefs/wp670k_brief.json`을 입력한다.
4.  출력 폴더 경로 `output/wp670k`를 입력한다.
5.  브랜드 아이덴티티 및 컬러 팔레트를 생성한다.
6.  로고 이미지 생성 여부 질문에서 `y` 또는 `n`을 입력한다.
7.  생성 결과를 `output/wp670k` 폴더에서 확인한다.
...
`wp670k_brief.json`과 출력 폴더를 입력한 후 기존 브랜드 분석 결과와 컬러
팔레트를 불러오고, 이미지 API를 이용하여 로고 이미지 생성을 실행한
화면이다.

![브랜드 브리프 입력 및 AI 로고 생성 실행 화면](images/page_14_image_03.png)

### 그림 2. 로고 이미지 생성 완료 화면

`logo_01.png`, `logo_02.png`, `logo_03.png`가 순차적으로 생성되었으며,
최종적으로 성공한 로고 수 **3/3**과 프로그램 실행 완료 메시지를
확인하였다.
...
WP-670K 제품의 브랜드 브리프 JSON 파일을 입력하여 브랜드 아이덴티티 생성
프로그램을 실행하였다. 생성된 브랜드 결과를 JSON 파일로 저장하고, 컬러
정보를 기반으로 브랜드 컬러 팔레트 이미지를 생성하였다. 이후 이미지 생성
API를 통해 서로 다른 콘셉트의 로고 3개를 생성하였으며, 모든 로고가
정상적으로 PNG 파일로 저장되었다.
...
**발생 가능한 상황:** 네트워크 연결 오류, API 서버 오류, 사용량·크레딧
문제, 이미지 생성 실패 등으로 OpenAI API 호출이 정상적으로 처리되지 않는
경우
...
사용자는 프로젝트의 `.env` 파일에 OpenAI API Key를 설정한 후 프로그램을
다시 실행해야 한다.
...
프로그램은 파일 오류, JSON 형식 오류, 입력값 오류, 저장 권한 오류 및 API
관련 오류를 예외 처리하여 사용자에게 원인을 알 수 있는 메시지를
출력하도록 구성하였다. 또한 로고 이미지 생성 과정에서는 개별 API 호출
실패가 전체 프로그램 중단으로 이어지지 않도록 하고, 오류를 출력한 뒤
다음 이미지 생성 작업을 계속 수행하도록 구현하였다.
...
제품명, 산업 분야, 타깃, 핵심 키워드 등 브랜드 생성에 필요한 정보가
부족한 경우 입력값을 검증하고, 부족하거나 비어 있는 항목에 대한 오류
메시지를 출력하여 사용자가 브랜드 브리프 JSON 파일을 확인하도록
안내한다.

### 필수 제품 특징을 찾지 못한 경우

제품 특징을 확인할 수 없는 경우 임의의 정보를 생성하여 사용하지 않고,
입력된 브랜드 브리프의 타깃·키워드·톤 등 확인 가능한 정보만을 기반으로
분석하도록 처리한다. 브랜드 생성에 반드시 필요한 필수 항목이 없으면 해당
항목을 명확하게 표시하고 사용자에게 입력 데이터 수정을 안내한다.

### AI 분석 결과 형식이 잘못된 경우

AI가 반환한 결과가 지정된 JSON 형식이 아니거나 JSON으로 변환할 수 없는
경우 `JSONDecodeError`를 처리하고 오류 메시지를 출력한다. 잘못된 분석
결과를 그대로 저장하거나 후속 작업에 사용하지 않도록 하여 컬러 및 로고
생성 단계에서 발생할 수 있는 추가 오류를 방지한다.

### 예외 처리 원칙

제품 분석 단계에서는 입력 데이터의 유효성을 먼저 검사하고, 확인되지 않은
제품 정보를 임의로 추가하지 않도록 한다. 또한 AI 응답의 JSON 변환 여부를
검사하여 정상적인 결과만 `brand_result.json`에 저장하고 후속 컬러 팔레트
및 로고 생성 단계에서 사용하도록 구성하였다.
...
-   **결과:** 브랜드명 4개, 슬로건 3개, 브랜드 스토리, 메인 컬러 1개와
    서브 컬러 3개를 생성하여 `brand_result.json`으로 저장하였다.
-   컬러 팔레트 PNG와 로고 이미지 3개를 생성하였다.
-   최종적으로 `"성공한 로고 수: 3/3"` 및
    `"프로그램 실행이 완료되었습니다."` 메시지를 확인하였다.

### 잘못된 JSON 테스트

-   **입력:** 괄호, 쉼표, 따옴표 등이 잘못되어 JSON 문법에 맞지 않는
    테스트용 브랜드 브리프 파일
-   **결과:** `JSONDecodeError`를 감지하여 다음 오류 메시지를 출력하고
    브랜드 생성을 중단하도록 처리하였다.

``` text
[JSON 오류] JSON 형식을 확인해주세요.
```

### 필수 필드 누락 테스트

-   **누락 필드:** `industry`, `target` 또는 `keywords` 중 하나
-   **결과:** `validate_brief()` 함수에서 필수 필드 누락 여부를
    검사한다.
...
OpenAI API 호출 과정에서 인증 오류, 네트워크 오류, API 사용량 또는
크레딧 문제 등이 발생하는 상황을 가정하여 예외 처리 동작을 확인하였다.
API 호출에서 예외가 발생하면 오류 내용을 사용자에게 출력하도록
구성하였다.

### 이미지 생성 실패 테스트

`generate_logo_images()`에서 각각의 로고 이미지 API 호출을 개별
`try-except`로 처리하였다. 특정 로고 생성에 실패하면 오류 내용과
`"다음 로고 생성을 계속합니다."`를 출력하고 프로그램 전체를 종료하지
않은 채 다음 로고 생성을 계속하도록 구현하였다.

정상 실행에서는 `logo_01.png`, `logo_02.png`, `logo_03.png`가 모두
생성되어 성공한 로고 수 **3/3**을 확인하였다.
...
## 29. 요구사항 충족 확인표
  ...
  요구사항                상태                    근거
  ----------------------- ----------------------- ------------------------------
  CLI 기반 Python         충족                    `print`/`input` 기반 실행
  프로그램                                        

  브리프 파일 경로 필수   충족                    JSON 경로 입력
  입력                                            

  출력 폴더 선택 입력 /   충족                    Enter 시 기본값
  기본 `./output`                                 

  `industry` / `target` / 충족                    `validate_brief()`
  `keywords` 필수                                 

  `tone` / `competitors`  충족                    선택 필드 검증
  / `notes` 선택                                  
  ...
  메인 1 + 서브 2\~3 /    충족                    메인 1 + 서브 3
  HEX                                             

  컬러 팔레트 PNG         충족                    `brand_palette.png`

  이미지 생성 API / 로고  충족                    `logo_01.png`\~`logo_03.png`
  2\~3개                                          

  `brand_result.json`     충족                    출력 폴더 저장
  저장                                            

  API Key 하드코딩 금지   충족                    `OPENAI_API_KEY` 환경변수

  API 실패 시 개별 로고   충족                    로고별 예외 처리
  계속                                            

  타깃 변경 비교          추가 구현               A/B 실제 결과 확인

  제품 PDF 직접 자동 분석 미구현                  사전 정리한 특징을 브리프에
                                                  반영

  경쟁사 자동 수집·분석   미구현/보너스 확장      `competitors` 입력 및 보고서
                                                  비교 방향 제시
  ------------------------------------------------------------------------------
...
Waterpik Ultra Professional WP-670K의 제품 자료에서 확인한 10단계 수압
조절, 치실·마사지 모드, 다양한 전용 팁, 30초 페이서와 1분 타이머 등의
주요 특징을 사전에 정리하고 브랜드 브리프에 반영하였다.

### ② 제품 특징의 브랜드 가치 해석

브랜드 브리프에 정리된 제품 특징과 핵심 키워드를 AI가 타깃 고객의
관점에서 해석하여 맞춤형 관리, 정밀한 세정, 효율적인 관리, 전문성, 신뢰,
편의성 등의 브랜드 가치로 연결하도록 구성하였다.

### ③ 타깃별 브랜드 생성 및 비교

동일한 WP-670K 제품이라도 입력된 타깃에 따라 서로 다른 브랜드
아이덴티티가 생성되도록 구성하였다. 실제로 20\~30대 직장인과 50\~60대
중장년층을 각각 타깃으로 설정하여 실행한 결과, 직장인 타깃에서는
클리오센스, 중장년층 타깃에서는 클린포커스가 생성되는 등 결과가 달라지는
것을 비교하였다.

### ④ 단계 간 생성 결과 연계

브랜드 브리프의 제품 특징과 타깃 정보를 AI 브랜드 생성 단계에 전달하고,
생성된 브랜드명·슬로건·스토리·컬러 결과를 컬러 팔레트 및 로고 생성
단계와 연계하였다.

### ⑤ 브랜드 브리프 입력값 검증

프로그램 실행 시 브랜드 브리프 JSON의 필수 항목인 `industry`, `target`,
`keywords`를 확인하고, 선택 항목인 `product_name`, `tone`,
`competitors`, `notes`도 입력되어 있는 경우 데이터 형식과 빈 값 여부를
검증하도록 구현하였다.

### ⑥ 기존 생성 결과 재사용

출력 폴더에 기존 `brand_result.json`이 존재하는 경우 이미 생성된 브랜드
결과를 다시 불러와 사용할 수 있도록 하였다. 이를 통해 불필요한 API
호출을 줄일 수 있도록 구성하였다.

### ⑦ 로고 생성 선택 기능

사용자가 프로그램 실행 과정에서 로고 생성 여부를 `y/n`으로 선택할 수
있도록 구현하였다. `n`을 선택하면 이미지 생성 API를 호출하지 않고 브랜드
결과와 컬러 팔레트만 처리할 수 있어 테스트 및 재실행 과정에서 불필요한
이미지 API 사용과 비용을 줄일 수 있도록 하였다.
...
## 31. 개발 과정에서 발생한 문제와 해결
  ...
  문제                    원인                     해결
  ----------------------- ------------------------ ------------------------------
  브리프 경로를 찾지 못함 입력 예시의 `>`까지      `>`를 제외하고
                          경로로 입력              `briefs/wp670k_brief.json`만
                                                   입력
  ...
  재사용                  `brand_result.json`      별도 출력 폴더 사용
                          재사용 로직              

  파일명 혼선             초기                     최종 제출 구조에서는
                          `clearfit_palette.png`   `brand_palette.png`로 정리
                          명칭과 최종 브랜드 결과  
                          혼재                     
  -------------------------------------------------------------------------------
...
생성된 브랜드 결과는 `brand_result.json`으로 저장하고, 브랜드 컬러는
Python의 `matplotlib`을 이용하여 `brand_palette.png`로 시각화하였다.
또한 OpenAI Images API를 이용하여 서로 다른 콘셉트의 로고 3개를
`logo_01.png`\~`logo_03.png`로 생성하였다. 동일한 제품에서 타깃 정보만
변경하여 실행함으로써 타깃에 따라 서로 다른 브랜드 결과가 생성되는 것도
확인하였다.

### 기존 과제와 비교해 개선된 점

단순히 산업·타깃·키워드를 입력하여 브랜드 요소를 생성하는 방식에서
확장하여, 실제 제품 자료에서 사전에 정리한 특징을 브랜드 브리프에
반영하고 AI가 이를 타깃 고객의 관점에서 브랜드 가치로 해석하도록
구성하였다. 또한 타깃 변경 A/B 테스트를 통해 동일한 제품에서도 타깃에
따라 브랜드 네이밍, 메시지와 컬러가 어떻게 달라지는지 비교할 수 있도록
하였다.

### 잘된 점

브랜드명, 슬로건, 스토리, 컬러, 로고가 하나의 브랜드 방향성을 공유하도록
각 생성 단계를 연계한 점이 잘 구현되었다. 특히 20\~30대 직장인을
대상으로 했을 때는 클리오센스, 50\~60대 중장년층을 대상으로 했을 때는
클린포커스가 생성되어 동일한 WP-670K 제품이라도 타깃에 따라 네이밍,
메시지와 컬러가 달라지는 것을 실제 결과를 통해 확인할 수 있었다.

또한 브랜드 브리프 입력값 검증, 기존 `brand_result.json` 재사용, 로고
생성 여부 선택 등의 기능을 추가하여 프로그램의 안정성과 사용 편의성을
높이고 불필요한 API 호출을 줄일 수 있도록 하였다.

### 아쉬운 점/한계

제품 PDF의 모든 내용을 프로그램이 직접 읽고 자동 분석하는 구조까지는
구현하지 못했으며, 제품 자료에서 사전에 확인한 핵심 특징을 브랜드
브리프에 정리하여 활용하였다. 따라서 현재 구현은 제품 자료 자체를 AI가
직접 분석하는 방식이 아니라 사전에 정리된 제품 특징을 AI가 브랜드
관점에서 해석하는 방식이라는 한계가 있다.

또한 이미지 생성 AI의 특성상 로고 내부의 영문 철자가 정확하지 않게
생성되는 경우가 있었으며, 이를 확인하고 수정하는 과정이 필요하였다.
이미지 API를 이용한 로고 생성에는 사용량에 따른 비용이 발생한다는 점도
고려해야 한다.

### 추후 개선 방향

향후에는 제품 PDF나 제품 설명 파일을 프로그램이 직접 입력받아 주요
특징과 USP를 자동으로 추출하는 기능을 추가할 수 있다. 또한 경쟁사 정보를
자동으로 수집·분석하여 자사 제품과의 차별화 요소와 브랜드 전략까지
생성하도록 확장할 수 있다.

로고 생성 이후 브랜드명의 영문 철자를 자동으로 검사하는 기능과 여러
타깃의 생성 결과를 한 화면에서 비교하는 기능을 추가한다면 프로그램의
정확성과 활용도를 더욱 높일 수 있을 것으로 생각한다.

### 제출 전 보안 점검

OpenAI API Key는 소스 코드에 직접 입력하지 않고 `.env` 파일을 통해
환경변수로 관리하였다. `.env`에는 실제 API Key가 저장될 수 있으므로 최종
제출용 압축파일에서는 반드시 제외한다. 가상환경인 `.venv` 역시 용량이
크고 실행 환경에서 다시 생성할 수 있으므로 제출 대상에서 제외한다.

최종 제출용 프로젝트에는 `main.py`, `requirements.txt`, `.gitignore`,
`briefs/`, `output/` 등 프로그램 실행과 결과 확인에 필요한 파일을
포함하고, `.env`와 `.venv`는 포함하지 않는다.
